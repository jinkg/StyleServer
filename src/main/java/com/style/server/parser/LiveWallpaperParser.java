 ...
 * on 2017/7/28.
 */
public class LiveWallpaperParser {
    ...
    private static LiveWallpaperItem parseFields(String[] wallpaperFields) {
        if (wallpaperFields != null && wallpaperFields.length == FIELD_COUNT) {
            ...
            item.price = Float.parseFloat(wallpaperFields[7].trim());
            item.pro = Integer.parseInt(wallpaperFields[8].trim()) == 1;
            item.minVersion = Integer.parseInt(wallpaperFields[9].trim());
